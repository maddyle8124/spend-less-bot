...
def get_bucket_status(bucket_id: str, month_key: str) -> dict:
    ...
    for r in rows:
        ...
        spent += amt if direction == "out" else -amt
    spent = max(spent, 0)
...
def get_default_buckets() -> list[dict]:
    return [
        ...
        {"id": "other",            "name": "📦 Other",             "daily_cap": None},
    ]
...
def append_transaction(tx_date, description, amount, ref_code, month_key, direction: str = "out") -> int:
    ws = _sheet(S.TRANSACTIONS)
    next_row = len(ws.col_values(2)) + 1  # col B is 1-indexed as 2

    loai = "Tiền vào" if direction == "in" else "Tiền ra"
    row_data = [
        "",           # A: ID
        ...
        "FALSE",      # N: Confirmed
        month_key,    # O: Month
        "",           # P: Notes (user-managed)
        "",           # Q: special occasion (user-managed)
        direction,    # R: Direction (in/out)
    ]
    ws.update(f"A{next_row}:R{next_row}", [row_data])
    print(f"DEBUG append_transaction: wrote row {next_row}, amount={amount}, direction={direction}")
    return next_row
...
def void_transaction(row_num: int):
    """Mark a transaction as voided — removes it from all bucket totals.
    Sets N (Confirmed) = FALSE and tags K (Category) = VOIDED so it's
    distinguishable from a plain orphan in future audits.
    """
    ws = _sheet(S.TRANSACTIONS)
    ws.update(f"K{row_num}:N{row_num}", [["VOIDED", "", "FALSE", "FALSE"]])
