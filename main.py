...
from handlers.transaction import handle_parent_selected, handle_sub_selected, handle_freetext_sub, handle_recategorize, handle_void_transaction, handle_keeprow
...
from handlers.reports     import send_monthly_status, send_today_status, run_weekly_summary, run_monthly_report, send_daily_recap, handle_daily_excuse, run_audit
from handlers.incoming    import handle_income_type_selected, handle_refund_bucket_selected

app = FastAPI(title="maddy spend less pls")


@app.on_event("startup")
async def on_startup():
    await tg.set_my_commands()
    sh._get_spreadsheet()  # warm up Sheets connection so first transaction isn't slow
...
@app.get("/")
async def health():
    return {"status": "ok", "bot": "maddy spend less pls"}
...
async def _process(body: dict):
    try:
        ...
        await tg.send_text(f"⚠️ Bot error: `{e}`")
...
async def _handle_callback(cb: dict):
    ...
    if prefix == "p":
        ...
        await handle_void_transaction(parts, message_id)
    elif prefix == "keeprow":
        await handle_keeprow(parts, message_id)
    elif prefix == "inc":
        await handle_income_type_selected(parts, message_id)
    elif prefix == "refbkt":
        await handle_refund_bucket_selected(parts, message_id)
...
async def _handle_command(text: str):
    ...
    elif cmd == "/audit":    await run_audit()
    elif cmd == "/log":      await _handle_log(parts)
    else:
        await tg.send_text("Unknown command. Try /status, /today, /allocate, /weekly, /report, /audit, or /log.")


async def _handle_log(parts: list[str]):
    if len(parts) < 2:
        await tg.send_text("Usage: `/log <amount> <description>`\nExample: `/log 25000 grab sáng nay`")
        return
    try:
        amount = float(parts[1].replace(",", "").replace(".", ""))
    except ValueError:
        await tg.send_text("❌ Amount không hợp lệ. Ví dụ: `/log 25000 grab sáng nay`")
        return
    description = " ".join(parts[2:]) if len(parts) > 2 else "Manual entry"
    from handlers.sepay import handle_sepay_webhook
    await handle_sepay_webhook({
        "transferAmount": amount,
        "transferType":   "out",
        "description":    description,
        "referenceCode":  f"manual_{int(__import__('time').time())}",
    })
