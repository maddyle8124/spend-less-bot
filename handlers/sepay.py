...
async def handle_sepay_webhook(payload: dict):
    ...
    # Detect direction: explicit "in"/"credit" → incoming, everything else → outgoing
    is_incoming = (("in" in tx_type and "out" not in tx_type) or "credit" in tx_type) and float(raw_amount) >= 0
    direction = "in" if is_incoming else "out"
    print(f"DEBUG: tx_type={tx_type!r}, amount={raw_amount}, direction={direction}")

    description = (data.get("description") or data.get("content") or "No description").strip()
    ...
    row_num = sh.append_transaction(tx_date, description, amount, ref_code, month_key, direction)

    if direction == "in":
        from handlers.incoming import handle_incoming_webhook
        await handle_incoming_webhook(amount, description, row_num)
        return
