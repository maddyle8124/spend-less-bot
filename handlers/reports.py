...
async def run_audit():
    """
    Audit current month: read straight from sheet (bypasses cache),
    show real spent per bucket, and list any unfiled transactions.
    """
    tz        = pytz.timezone(TIMEZONE)
    now       = datetime.now(tz)
    month_key = sh.fmt_month(now)

    # Force fresh read — ignore any stale in-memory cache
    ...
    for i, r in enumerate(rows[1:], start=2):
        ...
        if r[14] != month_key:
            continue
        amount = sh._parse_amount(r[7])
        if amount <= 0:
            continue

        parent    = (r[10] or "").strip()
        confirmed = str(r[13]).upper() == "TRUE"
        direction = r[17] if len(r) > 17 and r[17] else "out"

        if parent == "VOIDED":
            continue

        if confirmed and parent:
            delta = amount if direction == "out" else -amount
            spent_by_bucket[parent] = spent_by_bucket.get(parent, 0) + delta
            count_by_bucket[parent] = count_by_bucket.get(parent, 0) + 1
        else:
            orphans.append({
                "row":         i,
                "date":        r[1][:16] if len(r) > 1 else "",
                "description": r[5] if len(r) > 5 else "",
                "amount":      amount,
            })

    # ── Build report ─────────────────────────────────────────────────
    msg  = f"🔍 *Audit — {month_key}*\n"
    msg += f"_(numbers read directly from spreadsheet)_\n\n"

    total_spent = 0
    total_alloc = 0
    for b in buckets:
        spent = spent_by_bucket.get(b["id"], 0)
        alloc = b["allocated"]
        remaining = alloc - spent
        pct   = sh.calc_pct(spent, alloc)
        total_spent += spent
        total_alloc += alloc

        ico = "🔴" if pct >= 100 else "🟡" if pct >= 80 else "✅"
        n   = count_by_bucket.get(b["id"], 0)
        msg += f"{ico} *{b['name']}*\n"
        msg += f"   Spent: *{sh.fmt_amount(spent)}* / {sh.fmt_amount(alloc)}"
        msg += f"  ({n} tx)\n"
        msg += f"   Left:  {sh.fmt_amount(remaining)}\n\n"

    # Show unknown bucket IDs not in active config
    known_ids = {b["id"] for b in buckets}
    for bid, spent in spent_by_bucket.items():
        if bid not in known_ids:
            n = count_by_bucket.get(bid, 0)
            msg += f"⚠️ `{bid}` _(not in budget)_: {sh.fmt_amount(spent)} ({n} tx)\n"

    msg += f"─────────────────────\n"
    msg += f"Total spent: *{sh.fmt_amount(total_spent)}* / {sh.fmt_amount(total_alloc)}\n"

    if orphans:
        msg += f"\n⚠️ *{len(orphans)} unfiled transaction(s) — tap to categorize:*"
    ...
    for o in orphans:
        text = (
            f"🗂️ *{o['date']}* · -{sh.fmt_amount(o['amount'])}\n"
            f"`{o['description']}`"
        )
        buttons = [[{"text": "🔄 Categorize", "callback_data": f"recat_{o['row']}"}]]
        await tg.send_with_buttons(text, buttons)
...
async def run_weekly_summary():
    ...
    for r in rows:
        ...
        delta     = amt if direction == "out" else -amt
        by_bucket[parent] = by_bucket.get(parent, 0) + delta
        key = f"{parent}||{sub}"
        by_sub[key] = by_sub.get(key, 0) + delta
...
async def run_monthly_report():
    ...
    this_txns = [r for r in rows if len(r) >= 15 and r[14] == month_key and str(r[13]).upper() == "TRUE" and (r[17] if len(r) > 17 else "out") == "out"]
    prev_txns = [r for r in rows if len(r) >= 15 and r[14] == prev_key  and str(r[13]).upper() == "TRUE" and (r[17] if len(r) > 17 else "out") == "out"]
